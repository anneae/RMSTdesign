...
The accrual period is set to  $k_1$= 3 years and the followup period to  $k_2$= 5 years, and $\tau$ is equal to 7.5 years. We can use `RMSTpow` to determine the sample size needed to achieve 90% power with a one-sided type I error level of 0.025 (the default value). 
```{r}
RMSTpow(gogC, gogT_PH, 3, 5, 7.5, power = 0.9)
```

The required sample size is 436. We have a 96% chance of being able to estimate the difference in mean survival time using the unmodified area under the Kaplan-Meier curve, which is reasonable. 
...
mytest1
```
We see that the empirical power was 89.7%, and empirical probability the unmodified Kaplan-Meier estimator could be used was 0.968. The mean truncation time used in practice was 7.49 years with a standard deviation of 0.03.
...
We see that the numbers obtained match Table 2. If we had instead used a hazard ratio of 0.71, exactly what was observed in the GOG111 trial, the sample size required would have been larger.
```{r}
