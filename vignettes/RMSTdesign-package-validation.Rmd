...
This vignette validates the internal functions of the `RMSTdesign` package by comparing the results to by-hand calculations in the simple setting of exponential survival distributions and no censoring before time $\tau$. This vignette also confirms that the asymptotic power and pKME returned by the `RMSTpow` function are close to their empirical conterparts across several scenarios. Along with the results in our paper[^1] (Tables 1 and 3 which show that the power of the RMST-based test and pKME calculated using our package match their empirical counterparts across 4 scenarios, various values of k1, k2, tau and n, and a table in Appendix I which shows that the empirical variance of RMST matches the variance calcuated using our package), the vignette "Compare-to-SSRMST" (which shows that our results match simulation results from the `SSRMST` package[^2] in 4 scenarios), and the vignette "GOG111-Trial", which shows that we get almost the same sample size as the one calculated by Royston and Parmar[^3] (with the difference likely due to different approaches for calculating the restricted mean survival difference and its variance), these results validate that the calculations in the `RMSTdesign` package are correct and that the package can be used for trial design.  
...
$$\mu=\frac{\frac{1-p^r}{r}-(1-p)}{\frac{1}{\sqrt(n/2)}\sqrt(\frac{1}{r^2}(1-p^r\{p^r+2r\}))}$$
...
The `RMSTeval` function calculates the probability the difference in the restricted mean survival time at time $\tau$ will be estimable using the Kaplan-Meier estimator, equation 4 in our paper. Consider a very simple situation where survival in each group is eqaul to 1 until time $\tau$ and $\tau<k_1$. Then we get the probability $p_{KME}$ is estimable in each group is eqaul to 
$$ 1-\int_0^\tau \left(\frac{x}{k_1}\right)^{n-1}\frac{1}{k_1}dx=1-\left(\frac{\tau}{k_1}\right)^n$$
In the two-sample setting, the number of people in each group is n/2. The probability that we can estimate the RMST in both groups with the Kaplan-Meier estimator in both groups is the product of the probability we can estimate it in each group:
...
##Simulation notes
We do not use our own function `RMSTpow` with `sim=T` to do the simulations below; rather we use `rpwe` from the `PWEALL` package and `rweibull` to generate the simulated data for piecewise expoenential and Weibull distributions, respectively. We use `summary(survfit())` with the `rmean` option to calculate the RMST difference estimate and its estimated variance. Then, we calculate the Z statistic and compare it to a standard normal distribution to reject or fail to reject the null hypothesis. 

##Simulations for scenario 4, but varying the hazard rate and hazard ratio 
The plots in Figure S9, Scenario 4 show the asymptotic power across a range of control group hazard rates and hazard ratios, including the scenarios used below. In the original Scenario 4, the hazard rate was about 0.07 and the hazard ratio was 0.67 for the first 15 months, then 1.2. Below, we will consider 4 values of the hazard rate and 4 values of the hazard ratio over the first 15 months. The hazard ratio after 15 months will remain 1.2. Empirical quantities will be calculated using 1000 simulations. 
```{r}
